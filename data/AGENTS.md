...
- Presentation-only organism identity records under `data/presentation/` may authorize a coarse representative silhouette only when they carry explicit source-backed provenance and limitations. They are not simulation parameters, population calibration, or physical microscopy geometry; renderer/runtime code must not infer morphology from a taxon string when the record is absent.
