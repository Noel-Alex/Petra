...
- Product-default run initialization lives under `data/run_presets/**` and remains distinct from biological mechanism authority. A run preset must bind exact engine/protocol/scenario/parameter-set identity, classify deterministic seed/resource/inoculum choices honestly, and carry explicit usage/warning text. Stale identity is a refusal, not permission to rebind silently; engineering run-state choices are not measured culture conditions or outcome guarantees.
