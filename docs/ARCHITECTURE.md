...
| Separate division/death rate fields | **Pending #605** | Kernel quantities exist, but product render channels are not authority until the active transaction-bound projection/parity work lands | UI may reserve controls but must keep them unavailable until the merged render fields are present | Reconstructing either rate from net growth, endpoint deltas, opacity, or glyph churn |
