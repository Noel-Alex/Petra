...
- mechanism parameter-set hash;
- exact run-condition identity/fingerprint for authoritative initialization (resource/founders), separate from seed;
- scenario/version;
- canonical unsigned-32-bit simulation seed (the same numeric identity accepted by the mechanistic RNG/replay layer);
- intervention timeline;
- state normalization metadata;
- explicit input-feature schema version and target/output schema version;
- snapshot interval;
...
- train/validation/test splits by **mechanism/scenario/run-condition/intervention groups**, not adjacent frames or seed replicas from the same biological condition.

Avoid leakage where snapshots from one trajectory land in both training and validation.

Implementation contract: `src/ml/dataset.ts` assigns splits deterministically at the declared mechanism/scenario/run-condition/intervention **group** boundary, deliberately excluding snapshot time/index and seed from the split hash. `parameterSetHash` remains mechanism/provenance identity; exact initial resource/founder state is a separate versioned run-condition fingerprint derived from authoritative composed initialization. Equivalent run conditions cannot be duplicated under different display ids. `src/ml/sweep.ts` first assigns and counts complete groups, then applies a separately versioned held-out **coverage policy** before accepting any trajectory tasks. The default coverage contract requires at least one group in train, validation, and test; insufficient coverage is an explicit planning refusal with observed group/trajectory counts, not a reason to move seeds or frames between splits. Only after coverage passes does the planner expand caller-declared mechanism parameter points × run conditions × intervention families × canonical uint32 stochastic seeds into stable future mechanistic trajectory tasks. Seed is excluded from the split-group hash but remains part of exact trajectory identity. Arbitrary strings such as `"01"` are not accepted as alternate spellings of numeric seed `1`; the runner and ML manifest share the simulator's numeric seed domain. An explicit trajectory cap is also required. Generation manifest v5 records explicit run-condition id/fingerprint, canonical numeric seeds, explicit input/target schema identity, both split-assignment and split-coverage policy versions, and group/trajectory split counts. Schema identity is part of task/data provenance and task IDs, but it does not change the biological group-level split assignment. Surrogate benchmark evidence v5 is constructed from generated dataset provenance and binds its input/target schema identity, coverage-policy version, plus a separately versioned evaluation-weighting policy, preventing older evidence from being silently reinterpreted after either policy changes. Held-out records carry stable group + trajectory identity and an explicit requested forecast horizon; candidate and baseline predictions are paired on the exact same authoritative target row. The default evaluation contract accepts one record per trajectory × declared horizon, equal-weights horizons within a group, then equal-weights held-out groups overall, so denser snapshot cadence or longer trajectories cannot silently dominate promotion. Promoted model cards and benchmark evidence also share a versioned compatibility identity: explicit supported scenario/version pairs, normalization profile, input schema, and target schema. Runtime Emulated admission must match that identity exactly before OOD checks; multi-scenario compatibility is enumerated rather than inferred from feature overlap. Parameter-set hashes remain governed by the validated domain envelope when a surrogate intentionally spans parameter ranges. The sweep planner does not generate synthetic biology or imply the composed mechanistic engine is ready. After authoritative runners execute every planned task, `src/ml/generator.ts` is the dataset artifact gate: it requires one complete result per planned trajectory, exact task/trajectory/dataset/normalization identity, contiguous snapshot indices, monotonic simulation time, and a terminal reason on the final sample. Parallel completion order does not affect artifact order. Dataset artifact/row v4 records the same canonical numeric seed plus explicit run-condition id and exact input/target schema identity and exports rows as canonical JSONL with a stable provenance sidecar, and non-finite/non-JSON payloads are refused rather than coerced; large writers should consume the line iterator incrementally. A plan/manifest alone remains non-evidence, while an artifact proves collection completeness and identity—not biological validity or model quality. For spatial or otherwise large sweeps, `src/ml/incrementalGenerator.ts` avoids retaining the full artifact in RAM: each completed trajectory is revalidated and streamed into injected durable staging, tagged with an exact-plan digest plus row count/checksum, and may be resumed idempotently after interruption. Final publication rereads staged trajectories in canonical plan order, verifies every staged checksum/count, and publishes a versioned completion record only after all rows succeed. Partial output without that completion record is non-final/non-promotable and must be replaced rather than appended on retry; these deterministic checks detect ordinary truncation/duplication/corruption but are not cryptographic authenticity evidence. `src/ml/runtime.ts` owns the optional Emulated-mode feature/promotion/compatibility/OOD gate, and any failed gate falls back to Mechanistic execution. These helpers are infrastructure only; they do not mean a surrogate has been trained or promoted.
