...
- Multi-species breadth now has a named first bacterial implementation handoff. #873 selected and documented `Bacillus subtilis 168 trp+ sigE-` as the first shared-resource competitor evidence pack; #890 owns the authoritative E. coli MG1655 + B. subtilis scenario. Current physical glucose/gCDW values are calibration targets, not permission to relabel Petra `model-resource` / `model-biomass`. Fungal and additional-drug work remain separately evidence-gated.
...
- Protocol v8 / composed-state v5 is the current continuation contract for the next #809/#5 evolution transaction. Composed checkpoints now carry detached Petra RNG continuation state seeded from `RunIdentity.seed`; protocol/bundle restore must fail closed on malformed RNG state. This RNG is biological replay authority only—render/UI code must never consume it. Mutation sampling is still a subsequent integration step, not implied merely by the presence of RNG state.
