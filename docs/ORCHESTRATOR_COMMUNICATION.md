...
- Recovered the useful two-commit `Noel-Alex/457-composed-dish-projection` donor after it became stale/no-PR and 19 commits behind `main`; the donor and intermediate recovery branch remain reference-only rather than being merged wholesale.
...
- Accepted ciprofloxacin intervention geometry is now fully bound through the live renderer transaction: #822/#830 preserve exact global/radial/stripe/paint geometry, #841 binds runtime-history identity, and #855 carries the detached footprints on `DishRenderSnapshot`. UI/Pixi consumers should use `acceptedInterventionFootprints` directly rather than rereading protocol history or inventing point markers. #901 owns the remaining causal presentation layer; a footprint is an accepted-action marker, not efficacy, diffusion, clearance, affected population, or clinical delivery.
- Local ecology division/death/net-growth data is now live through the authoritative dish transaction. #838 carries the exact accepted step-local observation through composed engine → protocol/Worker/runtime with branch/position validation and stale-observation clearing; #878 appends the validated `net-growth` `RenderField` only on the matching accepted ecology-step dish snapshot. UI/Pixi may consume that field directly with `model-biomass/hour` semantics and snapshot-extrema presentation. Never reconstruct growth/death from endpoint deltas, animation speed, glyph churn, or renderer state.
...
- Colony presentation has moved onto the bounded texture path required by the laptop performance report: #867/#889 provide exact zero-opacity at zero lineage density, shared cross-lineage normalization, monotonic density→mass alpha, and a single linearly filtered density raster. #870 owns the still-active bounded connected high-density accent/island layer. Do not restore one Pixi object per occupied cell or an unbounded blob count; browser responsiveness remains measured under #850/#59/#869.
- Multi-species breadth now has a named first bacterial implementation path. #873 selected `Bacillus subtilis 168 trp+ sigE-`; #890 binds it with E. coli MG1655 through exact replay-critical taxon revisions and shared `model-resource`/local-capacity ecology. The v1 species growth distinction is an explicit dimensionless 0.67/0.69 isolated-growth-rate ratio from Tännler 2008 vs LaCroix 2015; it is not a measured co-culture coefficient. Physical glucose/gCDW yield, Monod K, direct interactions, Bacillus ciprofloxacin/evolution, and sporulation remain unavailable. UI/render work must consume the exact taxon map through the merged #946 join and stay morphology-neutral when exact presentation evidence is absent; do not infer species form from lineage names/colors.
...
## 2026-09-25 — product discovery, first-step runtime unblock, and next UI/data gates

- #1048 / PR #1050 repaired the taxon-bearing composed runtime's first-step failure by propagating the existing authoritative `config.taxonRegistry` into stepping-time founder reconstruction. The defect had blocked flagship smoke, ciprofloxacin validation, replay/calibration/render-parity/ML profiles, two-bacterium validation, and browser playback after successful initialization. The fix is on `main`, but this connector session did not execute the laptop workloads; rerun the registered local experiments before treating those paths as newly measured passes.
- #600 / PR #1049 adds `src/app/contentDiscovery.ts` as the framework-neutral product-library projection over the exact #879 support matrix plus bundled scenario discovery. Product-library support, grounded Science-Mode admission, and executable Sandbox registration are intentionally separate authorities. UI code should consume `listProductContentDiscovery()` for organisms/environment/interventions/limitations and blocked expansion reasons; it must not infer support from names, tags, morphology, or literature records, and discovery records intentionally carry no hidden `runtimeId`.
- #879 support matrix v1.0.4 reflects the current first two-bacterium gate after #992: the exact inert pack `ecoli-bsubtilis-shared-resource-pack@1.0.0` exists, while #907 still owns provenance-complete mechanistic/replay validation, #946 owns the remaining exact presentation-binding hardening, and #999 owns evidence-gated Sandbox/product promotion. Physical glucose/gCDW mapping, direct interactions, Bacillus ciprofloxacin/evolution, and sporulation remain unsupported.
- #1054 / PR #1055 extends the existing `expo-browser-acceptance` registration with a later-run/post-growth camera stress: reset the real runtime, drive real 16× playback to a fixed accepted-command frontier, pause, then measure the same renderer-owned whole-dish and colony-zoom camera workload used by the initial baseline. Do not lower scientific grid fidelity, drop channels, move compute architecture, or invent an FPS threshold before the laptop result. The next evidence command remains `python run_local_experiments.py --only expo-browser-acceptance --push`.
- #552 / PR #1056 adds only a deterministic numerical conjugation-count sampler over caller-owned distinct eligible recipients and caller-owned transfer probability, with exact sparse-binomial acceleration and transactional RNG refusal. It is **not** a biological HGT/contact model and must not become selectable product biology. #551 still owns the exact R388/host source law, #914 owns physical area/count coupling for production HGT, and #552 still needs authoritative acquisition state/ancestry/checkpoint/replay before #983 activation.
- The heterogeneous-render rule remains unchanged: the standalone source-validation *Aspergillus niger* front is not a composed-runtime lineage merely because biological time matches. A live bacteria+fungus scene must wait for shared accepted composed runtime/checkpoint authority (#1005/#976); until then, UI may present the fungal validation mode separately but must not synthesize fungal biomass/density/resource/network channels into the bacterial dish.

**Local UI implementation rule:** bind the reference look to these exact authorities. The dish body comes from authoritative density/resource/drug/observation data; organism morphology comes only from exact taxon-backed presentation records; future content availability comes from `contentDiscovery.ts`; executable scenario selection comes from the Sandbox runtime registry. Presentation may be highly stylized and optimized, but unsupported biology stays visibly unavailable instead of being approximated by UI conditionals.
