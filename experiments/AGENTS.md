...
- `two-bacterium-shared-resource-validation` consumes the real #890 E. coli MG1655 + B. subtilis 168 trp+ sigE- run plan. Its v2 compact evidence binds exact scenario + composed parameter-set + taxon revisions; until #992 lands a standalone #586 manifest, `contentPackManifest` must remain explicit `null` with the `standalone-content-pack-manifest-is-unbound` limitation rather than inventing a pack id. This validation is model-unit mechanistic evidence, not physical co-culture calibration or product/Science-Mode admission.
