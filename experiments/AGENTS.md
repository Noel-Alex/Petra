...
- Mechanistic validation evidence must never reuse scenario identity as a missing content-pack identity. When standalone packaging authority is not yet registered, encode the pack as explicitly unbound/null so the expensive mechanistic run can proceed while promotion remains fail-closed; once a real pack exists, bind its exact id/version without changing the biological result.
- `mops-glucose-resource-calibration` may run a source-inventory acquisition stage before the numerical calibration is scientifically ready. That stage must pin the repository-owned Knapp Harvard Dataverse DOI/version, keep the complete remote inventory and any future raw source files under `PETRA_LOCAL_ARTIFACT_DIR`, emit only capped provenance/candidate metadata, and remain fail-closed until an exact 37 °C D-glucose source table/schema and uncertainty representation are reviewed. Filename/directory ranking is discovery only and never authorizes a parameter fit, a glucose interpretation of `model-resource`, or physical spatial transport.
