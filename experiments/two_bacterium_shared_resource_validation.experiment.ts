  ...
  it('records the complete real-engine validation contract', () => {
    ...
    try {
      ...
      recordControl(controls, 'shared-resource-local-capacity-coherence', () => {
        ...
        const resourceConsumed = initialResource - finalResource
        ...
        assert.ok(
          Math.abs(resourceConsumed - biomassProduced) <=
            Math.max(1e-5, Math.abs(resourceConsumed) * 1e-5),
          'engineering yield=1 profile must preserve the configured resource-to-biomass accounting',
        )
      ...
      recordControl(controls, 'bacillus-evolution-refusal', () => {
        ...
        return 'The scenario contains no mutation transitions and protocol v8 refuses an untyped Bacillus evolution command before engine dispatch.'
      })
