...
| Petra's baseline composed parameter set may use a zero per-hour loss hazard before ciprofloxacin is active | engineering composition identity | versioned `ecoli-ciprofloxacin-baseline-composed` record bound to the declared resource×drug loss policy | zero means **inactive incremental ciprofloxacin loss**, not a measured MG1655 background death rate; initial resource/inoculum remain run-state inputs |
