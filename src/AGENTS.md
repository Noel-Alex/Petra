...
- `contentSupportMatrix.ts` is framework-neutral compatibility/product-discovery policy above inert biological content-pack manifests and repository-owned science records. It may determine which exact content packs are selectable or blocked, but it is not simulation/checkpoint/replay authority and must not infer compatibility from labels, taxon names, UI cards, or renderer state. `enabled-science` requires explicit Science-Mode admission, a bound resource context, and exact taxon `id + contentVersion` authority for every supported organism; blocked queue entries carry coordination dependencies only, never hidden scientific defaults.
