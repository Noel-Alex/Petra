...
- `dishRenderSource.ts` is the single app-layer transaction owner for dish source identity + snapshot object identity. One mounted surface lazily creates at most one visual-demo fixture and passes that exact object to DOM overlay controls and Pixi. Snapshot presence never implies authority: `authoritative-snapshot | visual-demo | awaiting-authoritative-snapshot` is explicit renderer-level presentation identity, and Pixi must not generate a second fixture internally.
