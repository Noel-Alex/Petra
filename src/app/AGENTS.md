...
- The browser entrypoint injects `createFlagshipExperimentRuntime` as the default runtime factory. It must resolve the repository-owned versioned engineering run preset through `buildFlagshipComposedRunPlan()` and create a fresh real browser `WorkerSession` per React effect lifetime. `App` itself still accepts an omitted factory for SSR/tests and must remain visibly unavailable in that case; never fall back to the synthetic fixture path.
