...
- `ciprofloxacinInterventionAdapter.ts` is the only app-layer preview→protocol-v5 conversion authority. It requires caller-supplied, validated metadata for the exact antibiotic parameter key/label, `mg/L` unit, finite bounds/default, blend mode, and supported geometry set; it never derives dose metadata from MIC/PD records or supplies product defaults. Point geometry and unsupported tools/parameters fail closed rather than being reinterpreted.
