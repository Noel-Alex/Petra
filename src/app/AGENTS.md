...
- Sources is a non-modal disclosure, not a focus trap. While open, unconsumed Escape outside editable controls may close it from anywhere in the app; closing from inside the disclosure restores focus to the stable Sources trigger. `aria-expanded`/`aria-controls` must track the visible surface.
