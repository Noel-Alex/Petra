...
- Successfully deserialized Worker messages are still untrusted: browser transport exposes payloads as `unknown`, and only `parseWorkerResponse(...)` may promote them to typed response state. A malformed response clears active/queued work through the same terminal failure path while retaining correlation from the already-trusted active outbound request; never copy a command id from an unvalidated response into session authority.
