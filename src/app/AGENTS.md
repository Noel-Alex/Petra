...
- Dish keyboard Escape is scoped to the focused dish surface: active intervention cancellation gets first refusal, editable controls are exempt, then Escape may request presentation-only camera overview. It must not replace global playback shortcuts outside the dish.
