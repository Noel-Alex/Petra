...
- `runIdentityKey.ts` is the canonical app-layer serialization of exact run identity for onboarding, causal narration, and Presenter Mode. Do not fork field order or omit engine/protocol/scenario/parameter/seed identity in presentation adapters.
- Presenter Mode is presentation-only and binds through `presenterRuntime.ts` to the exact active run identity. Changing run identity, or dropping a previously seen authoritative snapshot during same-identity reset/replay reinitialization, clears cue progress/evidence. The current protocol may satisfy only `flagship-runtime-ready` when an authoritative snapshot exactly matches the versioned flagship scenario; `advanced`, `synthetic-pulse`, renderer glyphs, wall-clock timing, curated-but-not-active provenance, and unverified comparisons must not unlock scientific demo gates.
- Presenter shell entry motion uses the shared panel transition plan: Full may use the short navigational reveal, Reduced crossfades, and Off/static treatment stays non-spatial. Presenter cue motion remains owned by `PresenterGuide` and shared motion tokens.
