...
- Sources is a non-modal disclosure, not a focus trap. While open, unconsumed Escape on the Petra app keyboard path may close it outside editable controls; closing from inside the disclosure restores focus to the stable Sources trigger. `aria-expanded`/`aria-controls` must track the visible surface.
