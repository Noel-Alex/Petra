...
- `DishViewport` owns one mounted `dishRenderSource.ts` transaction: authoritative snapshots always win; explicit visual-demo mode lazily creates and caches one fixture object; DOM overlay controls and `PixiDish` must consume that exact resolved snapshot plus its explicit source identity. `PixiDish` must never generate a second demo fixture or infer scientific authority merely from snapshot-shaped data.
