...
- Dish camera controls are presentation-only. The visible overview/reset action should send a declarative reset request into the renderer rather than storing Pixi objects in app state.
- Escape inside the dish may reset camera overview only after an active tool/default-prevented/editable-control path has had first refusal; it must not override intervention cancellation or synthesize worker commands.
