...
- `dishOverlaySelection.ts` owns presentation selection identity for field overlays. `automatic`, explicit `none`, and a specific `field` are distinct states; never overload `null` to mean both automatic and no overlay. Explicit none survives snapshot updates. If a specifically selected field disappears, reconcile the persisted/effective selection to automatic so a later field reappearance cannot silently resurrect stale user intent.
...
- Escape is single-consumer presentation input. Editable or already-default-prevented controls are rejected before dish/tool first-refusal callbacks run. A higher-priority shell/tool owner that returns consumed must prevent default and stop propagation before camera reset or ancestor handling; dish overview reset is only the fallback owner. This arbitration must never synthesize worker commands.
