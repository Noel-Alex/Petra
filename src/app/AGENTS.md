...
- A successfully deserialized Worker message is not trusted merely because TypeScript names the transport. `WorkerSession` accepts `unknown`, promotes only through `parseWorkerResponse(...)`, and on malformed responses clears queued work through the same terminal path while retaining correlation from the already-trusted active outbound request. Never adopt a command id from an unvalidated response.
