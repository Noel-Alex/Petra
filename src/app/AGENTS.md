...
- Focused dish geometry must size the square renderer from **remaining viewport block space**, not raw viewport height alone: the shell also owns overlay controls and semantic guidance below the frame. Preserve a `vh` compatibility fallback plus `dvh` sizing, reserve explicit vertical chrome, and keep focused-stage vertical overflow scrollable rather than clipping mounted controls at short heights/zoom. Do not solve fit failures by hiding scientific/control surfaces.
