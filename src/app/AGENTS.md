...
- `DishViewport.tsx` may choose/display source-provided render overlays and units, but it must not derive scientific units or fabricate authoritative snapshots. Missing authoritative state defaults to a neutral waiting surface; visual-demo biology is explicit opt-in for presentation development/QA only and must remain visibly labelled visual-only. Overlay/legend/interaction chrome must never derive from a hidden demo fixture while product mode is awaiting authority.
