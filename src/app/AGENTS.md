...
- The product default runtime is injected from `src/main.tsx` through `defaultFlagshipRuntimeFactory`. It must resolve the repository-owned engineering run-initialization preset, validate exact engine/protocol/scenario/parameter-set identity before constructing a WorkerSession, and start composed authority only. The synthetic worker scaffold remains infrastructure/test-only and must never become product truth. Invalid or stale preset identity fails through the existing runtime-recovery path rather than silently rebinding.
