...
- Local-region inspection consumes only the current composed `ExperimentRuntime` snapshot. App-layer selection may carry normalized dish input plus exact run/checkpoint/trace identity, but scientific values come from `inspectAuthoritativeRegion()`, never renderer glyphs or overlay pixels. Synthetic/missing authority and run-identity drift fail closed; intervention placement owns dish input while active.
