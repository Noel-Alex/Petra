...
- experimentRuntime.ts composes control intent, WorkerSession state, authoritative command confirmation, and timeline projection. Commands become replay history only after a returned authoritative event confirms their command id. It also owns `runBranchIdentity`: one deterministic lifecycle generation per monotonic command-history scope. Reset, reseed, and replay rotate that identity before accepting the restarted history; renderer replay and causal narration must consume this same runtime value rather than minting their own.
