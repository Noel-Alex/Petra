...
describe("app keyboard runtime availability", () => {
  ...
  it("keeps playback, speed, and step shortcuts inside visible runtime gates", () => {
    expect(
      canDispatchAppShortcut({ type: "toggle-play" }, ready),
    ).toBe(true);
    expect(
      canDispatchAppShortcut(
  ...
  it("consumes global pause only when playback is actually running", () => {
