...
describe("authoritative composed dish projection", () => {
  it("projects exact authoritative spatial channels into presentation arrays", () => {
    ...
    const dish = projectAuthoritativeComposedDishSnapshot(simulation);
  ...
  it("keeps one sampling identity across a run while snapshot identity follows trace authority", () => {
    const engine = composedEngine();
    const firstSimulation = engine.snapshot();
    if (firstSimulation.checkpoint.authority !== "composed") {
      throw new Error("expected composed snapshot");
    }
    const first = projectAuthoritativeComposedDishSnapshot(firstSimulation);
    ...
    const second = projectAuthoritativeComposedDishSnapshot(secondSimulation);
  ...
  it("fails closed to no dish projection for synthetic or absent authority", () => {
    expect(projectComposedDishSnapshot(null)).toBeNull();
    expect(projectComposedDishSnapshot(syntheticSnapshot())).toBeNull();
  });
});
