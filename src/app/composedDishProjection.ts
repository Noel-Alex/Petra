...
export function projectComposedDishSnapshot(
  ...
  return projectAuthoritativeComposedDishSnapshot(snapshot);
}
...
export function projectAuthoritativeComposedDishSnapshot(
  ...
  const projected: DishRenderSnapshot = {
    ...
    samplingIdentity: samplingIdentity(
    ...
    fields,
    lineages,
    // Current protocol events have no authoritative dish coordinates.
    // Do not invent positions from command geometry or renderer state.
    events: [],
  };
