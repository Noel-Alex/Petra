...
export function parseFlagshipRunPreset(value: unknown): FlagshipRunPreset {
  ...
  if (!Array.isArray(record.inocula)) {
    throw new Error('flagshipRunPreset.inocula must be an array')
  }
  const inocula = record.inocula.map((value, index) => {
    ...
    return Object.freeze({
      ...
      x: requireSafeInteger(
        `flagshipRunPreset.inocula[${index}].x`,
        inoculum.x,
      ),
      y: requireSafeInteger(
        `flagshipRunPreset.inocula[${index}].y`,
        inoculum.y,
      ),
      biomass: requireFiniteNonNegative(
...
/**
 * Resolve the repository-owned default run state against the repository-owned
 * flagship mechanism authority. A run preset may choose state; it cannot
 * relabel the scenario or parameter set it initializes.
 */
export function buildDefaultFlagshipRun(): DefaultFlagshipRun {
  const preset = parseFlagshipRunPreset(runPresetData)
