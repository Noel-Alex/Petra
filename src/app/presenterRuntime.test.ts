...
function runtimeState(
  ...
  const identity = createRunIdentity({
    scenarioId: options.scenarioId ?? flagship.id,
    scenarioVersion: options.scenarioVersion ?? flagship.version,
  ...
  return {
    ...
    worker: {
      phase: options.phase ?? (snapshot === null ? "initializing" : "ready"),
...
describe("Presenter runtime authority adapter", () => {
  it("uses the complete authoritative run identity as its stable binding key", () => {
    const identity = runtimeState().controls.identity;
    const key = runIdentityKey(identity);

    expect(key).toContain(identity.engineVersion);
  ...
  it("does not call another scenario, a missing snapshot, initializing state, or an errored session flagship-ready", () => {
    expect(
      projectPresenterRuntime(
