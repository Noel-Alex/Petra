 ...
 * The radius is a query footprint in normalized dish coordinates; it is not a
 * biological parameter and never enters simulation state.
 */
export function createRegionInspectionRequest(
  ...
  if (!Number.isFinite(radius) || radius <= 0 || radius > 1) {
    throw new RangeError("region inspection radius must be finite and within (0, 1]");
  }
...
/**
 * Projects an inspector readout from the exact composed checkpoint currently
 * owned by ExperimentRuntime. Renderer glyphs, overlays and camera state never
 * participate in the scientific measurement.
 */
export function projectRegionInspector(
  ...
  try {
    const readout = inspectAuthoritativeRegion(
      snapshot.checkpoint.composedState,
      request.selection,
    );
    return {
      state: {
