...
describe("region inspector runtime coordinator", () => {
  ...
  it("accepts only the active request error and retains any prior readout as stale", () => {
    ...
    expect(failure.state.presentation).toMatchObject({
      ...
      staleReadout: { selectionId: "region-a" },
    });
  });
  ...
  it("copies accepted canonical selections without rewriting caller identity", () => {
    const selection = {
      id: "region-a",
      centerX: 0.4,
      centerY: 0.5,
    ...
    expect(started.request.selection).toEqual(selection);
    expect(started.request.selection).not.toBe(selection);
  });

  it("rejects non-canonical selection-id aliases instead of trimming them", () => {
    expect(() =>
      beginRegionInspectionQuery(
        createRegionInspectorRuntimeState(),
        { ...selectionA, id: " region-a " },
      ),
    ).toThrow(/canonical/);
  });
