...
- Worker lifecycle failures must fail closed: an unexpected exit rejects its active task, and a worker that cannot reach its initial `ready` state must reject bounded waiting work rather than entering an unbounded respawn loop.
