...
- The leakage boundary is the scenario + exact mechanism parameter identity + authoritative run-condition fingerprint + intervention fingerprint. Run condition is separate from both mechanism parameters and stochastic seed; every seed replica of one condition must remain in one split.
