...
- Worker modules may receive only structured-cloneable task/execution data. Resolver/projector functions and biology are not serialized ad hoc across threads; a concrete authoritative worker module must be bound to an exact versioned execution definition before broad dataset generation is unblocked.
