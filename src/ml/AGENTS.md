...
- `src/ml/executionDefinition.ts` owns the bridge from sweep identity to exact mechanistic execution. `parameterSetHash` is a legacy field name: authoritative composed sweeps must populate it with the versioned length-prefixed execution-point identity, which binds the provenance mechanism record (binding schema + authority + parameter-set id/version + composed configuration fingerprint) **and the exact initial resource/lineage-biomass run state**. Initial fields remain run state, not mechanism configuration, but dataset tasks must still commit to them so one task+seed cannot silently start from different biology. Use `createSweepParameterPointForBinding(..., config)` rather than hand-authoring it.
