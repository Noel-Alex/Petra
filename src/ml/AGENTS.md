...
- Dataset-generation manifest v3 records plan/dataset/engine/scenario identity, canonical numeric simulation seeds, split-assignment policy, split-coverage policy, per-split group/trajectory counts, and stable trajectory keys. These are execution provenance, not evidence that trajectories were actually simulated.
