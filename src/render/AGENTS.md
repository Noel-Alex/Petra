...
- Multi-touch pinch is presentation-only camera intent: track touch pointer identity deterministically, zoom around the gesture centroid through the shared camera helpers, and rebase cleanly when a pointer lifts/cancels. Scope `touch-action: none` to the live dish interaction surface; do not disable page gestures globally.
