...
- React adapters apply a render snapshot and its selected overlay as one coherent presentation transaction. Overlay validation targets the incoming snapshot; a previously valid selection that disappears clears without a stale frame, while a genuinely unknown incoming overlay remains an explicit error. Do not rely on passive-effect declaration order for snapshot/overlay consistency.
