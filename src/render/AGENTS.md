...
- Touch input uses Pointer Events with stable pointer identity. Two-pointer pinch zoom must reuse the bounded camera helpers, keep the gesture centroid visually anchored, transition cleanly back to one-pointer pan after lift/cancel, and keep browser touch-action suppression scoped to the interactive dish surface rather than the page.
