...
- Wheel zoom input must normalize `WheelEvent.deltaMode` (pixel/line/page) through the shared presentation policy before exponential zoom scaling. Do not treat raw `deltaY` as pixels, reintroduce device sniffing, or duplicate sensitivity constants in DOM/Pixi handlers; actual mouse/trackpad feel remains a #59 hardware/browser gate.
