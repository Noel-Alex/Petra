...
- Hyphal structures use the optional `DishRenderSnapshot.hyphalPaths` channel only. Every path carries explicit `organismKind: "fungus"` evidence plus stable normalized geometry; the renderer must not synthesize fungi from lineage density or decorative state. `hyphalPathPresentation.ts` may reveal only exact-prefix extension/new branches through the shared evidence-gated `fungal-branch` motion phase. Removal, shortening, or topology movement snaps to authority instead of fabricating biological retraction/morphing. Full motion may animate path length; Reduced/Off settle immediately to the exact authoritative geometry.
