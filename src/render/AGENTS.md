...
- Dense lineage regions may add `lineageDensityContours.ts` isocontours to make contiguous populations read as coherent colony masses. The contour input is the same square-root-compressed, snapshot-wide **shared** density scale used by aggregate marks; per-lineage self-normalization is forbidden. Default contour levels are dimensionless presentation intensities, not biomass/CFU/physical colony-front thresholds. Neutral contour halo bands consume the existing lineage `patternToken`, while hue remains secondary reinforcement. This generic density treatment must not be described as bacterial rods, fungal hyphae, budding morphology, or another organism shape without authoritative organism-kind evidence.
