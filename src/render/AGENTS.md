...
- Renderer backing resolution follows the shared capped-DPR policy in `rendererResolution.ts`. DPR changes use a disposal-safe resolution media-query watcher rather than per-frame polling; update Pixi `renderer.resolution` then reuse the coalesced resize/redraw scheduler. Resolution changes are presentation/device state only and must not reset camera or alter snapshots.
