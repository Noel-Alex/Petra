...
- Renderer lifecycle narration uses one stable polite/atomic status region containing status copy only. Focusable retry controls must remain outside that live-region subtree and may reference its explanation with ordinary accessibility relationships such as `aria-describedby`; do not remount a pre-populated interactive live region to announce failure. Visual failure copy may mirror the narration for sighted users but must not create duplicate screen-reader narration.
