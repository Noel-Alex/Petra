...
- Render-source identity is explicit and independent from snapshot object presence. `PixiDish` never creates demo snapshots or infers authority from a non-null snapshot; its caller supplies one coherent `authoritative-snapshot | visual-demo | awaiting-authoritative-snapshot` transaction, validated by `src/render/source.ts`. App-layer `DishViewport` owns fixture creation/caching so DOM controls and Pixi consume the exact same snapshot object.
