...
- Direct pointer/touch manipulation is immediate camera presentation state: one active pointer pans only beyond overview; two active pointers pinch around their centroid and may translate with the centroid. New **renderer-owned movement** cancels/rebases any in-flight camera transition at the currently rendered camera; merely placing one finger at overview does not cancel camera motion or claim page scroll.
- Touch ownership is latched when the first admitted pointer starts a gesture. At whole-dish overview the renderer host uses `touch-action: pan-x pan-y` for the next gesture so ordinary one-finger page panning stays browser-owned while browser pinch zoom remains withheld; beyond overview it uses `touch-action: none` so Petra owns one-finger pan plus custom pinch. Two-pointer pinch is Petra-owned at every zoom. Mid-gesture camera/touch-action changes never transfer the existing one-finger ownership latch; pointer up/cancel clears it when the last pointer leaves, and renderer destroy restores the host's prior touch-action.
