...
- Demo snapshots must be explicitly tagged as visual-only and dimensionless; they are never evidence, calibration, or a scientific preset. Product/runtime surfaces must never substitute demo biology for missing authoritative state: demo rendering is explicit opt-in and visibly disclosed as not simulation data. The owning app adapter resolves one snapshot transaction and passes both the exact snapshot object and explicit source identity into Pixi; `PixiDish` must not manufacture its own demo fixture or infer authority from snapshot presence.
