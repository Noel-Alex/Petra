...
/**
 * Converts one tap/click into normalized dish coordinates using the exact
 * renderer aperture + camera geometry. The point is presentation/input
 * geometry only until an authority-layer query consumes it.
 */
export function resolveDishActivationPoint(
  ...
  if (!Number.isFinite(maximumTravelPx) || maximumTravelPx < 0) {
    throw new RangeError(
      "dish activation maximumTravelPx must be finite and non-negative",
    );
  }
