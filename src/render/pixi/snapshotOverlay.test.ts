...
describe("snapshot + overlay transaction", () => {
  ...
  it("rejects an invalid initial overlay against the first incoming snapshot", () => {
    const nextSnapshot = snapshotWithFields("next", [nutrientId]);

    expect(() =>
      resolveSnapshotOverlayUpdate(
        { snapshot: null, overlayId: "missing-overlay" },
        nextSnapshot,
        "missing-overlay",
      ),
    ).toThrow(/unknown render overlay: missing-overlay/);
