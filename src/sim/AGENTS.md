...
- Serialized RNG checkpoint state is exact replay authority: every state word must already be an integer in `[0, 0xffffffff]`, and the four-word state cannot be all zero. Restore paths reject malformed/fractional/out-of-range/non-finite words rather than canonicalizing them with bitwise coercion.
- Synthetic engine checkpoint restore is fail-closed and atomic: `tick` and `commandCount` must be non-negative safe integers, `simulationTimeHours` must exactly equal the engine tick/time mapping, `syntheticPopulation` must be finite and non-negative, and replacement RNG state must validate before any live scalar/RNG/event mutation. A rejected restore leaves the pre-restore snapshot unchanged.
