...
- `composedEngine.ts` is the worker-facing replay/transport adapter over that scaffold. Composed initialization is explicit, run scenario identity must match the composed evolution scenario, snapshots/checkpoints deep-copy ordered lineage + genotype channel state, and composed mode rejects synthetic fixture interventions. Omitted composed configuration keeps only the synthetic infrastructure path.
