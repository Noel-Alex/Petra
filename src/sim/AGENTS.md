...
- `SimulationCheckpoint.commandCount` is the authoritative accepted mutating-command position already carried by protocol v2. Renderer replay may project it as branch-local keyframe order, but it remains distinct from biological time. Restore/reset/replay histories that restart or regress this count are new history scopes and must not be concatenated under one renderer branch identity.
