...
- Composed lineage IDs, genotype IDs, and evolution scenario ID/version are canonical replay identity: they must be non-empty strings with no surrounding whitespace. The composition boundary rejects aliases rather than trimming them, so the identity used for fitness resolution is byte-for-byte the identity fingerprinted and serialized.
