...
- Worker request/response payloads are `unknown` at runtime until `parseWorkerRequest()` / `parseWorkerResponse()` accept them. The protocol boundary must fail closed on malformed envelopes, nested identity/checkpoint/snapshot structure, sparse arrays, and non-finite numeric authority; do not coerce/default missing scientific values. Numeric protocol-version mismatch stays distinguishable from malformed shape.
