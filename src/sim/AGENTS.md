...
- Worker transport payloads are untrusted runtime data even when TypeScript names them `WorkerRequest` / `WorkerResponse`. `protocolRuntime.ts` is the single framework-neutral promotion boundary: validate the envelope and nested replay-critical protocol state before Worker/session/engine handling. Malformed requests must not reach `SimulationEngine`; malformed responses must not become authoritative snapshots. Runtime parsing does not replace engine/direct-call semantic invariants, and it must never coerce/default malformed scientific or replay identity into validity.
