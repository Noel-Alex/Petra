...
- Worker protocol v3 distinguishes infrastructure-only synthetic checkpoints from `authority: 'composed'` checkpoints. `ComposedSimulationEngine` is the worker-facing deterministic loop over `authoritative.ts`; it advances resource/biomass ecology, preserves ordered lineage/genotype identity, validates checkpoint aggregates before restore, and rejects synthetic fixture commands. Omitted `composedConfig` remains a narrow infrastructure/test path only.
...
- `validateComposedStateAgainstConfig()` in `authoritative.ts` is the single authority for scientific composed-state/config invariants shared by direct stepping and checkpoint restore. `ComposedSimulationEngine` delegates restored scientific state to it; checkpoint-only run identity, tick/time/count, and aggregate-metric consistency remain explicit engine-boundary checks.
