...
- `beginEcologyStep()` / `completeEcologyStep()` expose the reviewed interphase boundary for higher-level evolution composition: local growth/death/resource flux commits first, then a caller may only conservatively reassign already-produced biomass between lineage cohort channels in the same spatial cells, then the existing spread operator runs with the original pre-step capacity reservation. The compatibility `stepEcology()` wrapper is exactly begin→complete with no reassignment. Interphase mutation must not change mask, resource, dimensions, or per-cell total biomass beyond the existing Float32 representation envelope; composed callers that may refuse during mutation materialization must use detached state so the full transaction remains atomic.
