...
- `beginEcologyStep()` / `completeEcologyStep()` expose the only supported mid-step composition seam: after local division/death has been committed but before coarse spread. Interphase callers may conservatively reassign already-committed biomass among lineage cohorts only; dimensions, mask, resource, and per-cell total biomass are guarded, and the preserved pre-death capacity reservation still governs spread. `stepEcology()` remains the no-interphase compatibility wrapper. This seam exists for authoritative mutation child-cohort insertion; it is not permission to create biomass or bypass `populationAuthority.ts`.
