...
export class SimulationEngine {
  ...
  constructor(identity: RunIdentity) {
    ...
    this.events.push({ sequence: 0, tick: 0, simulationTimeHours: 0, type: 'initialized' })
  }

  execute(command: SimulationCommand): SimulationSnapshot {
    if (command.type === 'restore') {
      this.restore(command.checkpoint)
      this.events.push({ sequence: this.events.length, tick: this.tick, simulationTimeHours: this.currentSimulationTimeHours(), type: 'restored', commandId: command.id })
    ...
    if (command.type === 'advance') {
      ...
      this.commandCount += 1
      this.events.push({ sequence: this.events.length, tick: this.tick, simulationTimeHours: this.currentSimulationTimeHours(), type: 'advanced', commandId: command.id, value: command.ticks })
      return this.snapshot()
    }
    ...
    this.commandCount += 1
    this.events.push({ sequence: this.events.length, tick: this.tick, simulationTimeHours: this.currentSimulationTimeHours(), type: 'synthetic-pulse', commandId: command.id, value: command.magnitude })
    return this.snapshot()
  }
