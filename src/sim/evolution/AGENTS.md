...
## Runtime lineage ecology baseline target authority
- `runtimeLineageEcologyBaseline.ts` owns the preparatory replay-oriented target for per-runtime-lineage organism ecology baselines needed by later external inoculation. It aligns one-to-one with explicit lineage-origin v2 creation order and persists the resolved positive baseline growth-rate scale, non-negative baseline non-drug death hazard, and exact static source-lineage-definition identity instead of re-deriving those values from genotype/taxon names or ancestry at every ecology step.
- Configured founders resolve from their exact configured lineage definitions (legacy omitted growth scale resolves once to neutral `1`); mutation children inherit the exact parent organism growth/source authority while baseline loss continues to resolve only through the reviewed dynamic-loss policy; an external-inoculation root may enter only through an exact #1059 admission whose source definition and admitted growth/loss values still match the active configured authority.
- The legacy v1 preparation helper deliberately composes with the explicit lineage-origin migration and therefore refuses ambiguous later parentless roots. Pure mutation/external append helpers require exactly one origin append and preserve prior origin history byte-for-byte before extending ecology authority.
- This target is **not yet live composed checkpoint/Worker authority** while #918 owns protocol/replay migration. Do not add required fields silently to protocol v8 / composed state v5, do not infer CFU/cell-equivalent quantity, and do not make #980 executable until origin + ecology baseline + taxon/biomass/population channels can publish atomically under the coordinated wire version.
