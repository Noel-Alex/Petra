...
describe('lineage parent lifetime chronology', () => {
  ...
  it('rejects retroactive parent extinction before an existing child birth atomically', () => {
    const registry = new LineageRegistry()
    const parent = registry.create({
      parentLineageId: null,
      genotypeId: 'WT',
    ...
    expect(() => registry.markExtinct(parent.lineageId, 1)).toThrow(
      /cannot precede an existing child creation/,
    )
    expect(registry.checkpoint()).toEqual(before)
  })

  it('allows equal-time child creation and parent extinction deterministically', () => {
    const registry = new LineageRegistry()
    const parent = registry.create({
      parentLineageId: null,
      genotypeId: 'WT',
    ...
    expect(
      registry.eventLog().map(({ kind, lineageId, timeHours }) => [
        kind,
        lineageId,
        timeHours,
      ]),
    ...
    expect(LineageRegistry.restore(registry.checkpoint()).checkpoint()).toEqual(
      registry.checkpoint(),
    )
  })

  it('rejects checkpoints whose child outlives the recorded parent lifetime boundary', () => {
    const registry = new LineageRegistry()
    const parent = registry.create({
      parentLineageId: null,
      genotypeId: 'WT',
    ...
    const corrupted: LineageRegistryCheckpoint = {
      ...
      records: checkpoint.records.map((record) =>
        record.lineageId === parent.lineageId
          ? { ...record, extinctAtHours: 0.5 }
          : record,
      ),
      events: checkpoint.events.map((event) =>
        event.kind === 'lineage-extinct' &&
        event.lineageId === parent.lineageId
          ? { ...event, timeHours: 0.5 }
          : event,
      ),
    }

    expect(() => LineageRegistry.restore(corrupted)).toThrow(
      /after its parent extinction/,
    )
  })
})
