...
- A valid preview yields an `apply-intervention` **UI intent**, not automatically a worker command. Protocol v9 exposes `apply-ciprofloxacin` plus engineering-unit `apply-model-resource` for replayable global/radial/stripe/paint edits, but a UI adapter may convert a preview only when authoritative scenario/runtime metadata supplies the exact parameter semantics, bounds/default/precision and allowed geometry. Resource conversion must preserve exact `model-resource` units and must never relabel them as glucose/nutrient/medium or a physical concentration. Unsupported tools/geometries remain uncommittable; adapters must never substitute `synthetic-pulse` or another fixture command.
...
- `AuthoritativeRegionInspection` carries exact checkpoint `RunIdentity`, tick, `simulationTimeHours`, and accepted command position. The state machine must preserve those fields unchanged with the readout through ready/stale/error-retained states; adapters must never substitute the current run/time for an older stale result or reconstruct biological time from renderer/animation cadence.
- `RegionInspectorPanel.tsx` is the reusable DOM readout for this lifecycle. It may display only `AuthoritativeRegionInspection` results, must label stale/error-retained results with their original selection identity, and must display biological time/tick/command position only from the supplied checkpoint-bound readout. It must keep `model-biomass` / `model-resource` explicit rather than upgrading them into physical units. Lineage rows may display the exact authoritative `genotypeId` supplied by the simulation projection, but UI code must not infer resistance, fitness, phenotype, confidence, or treatment response from that identifier alone. The typed `no-grid-coverage` result is an authoritative absence-of-measurement outcome: present it explicitly and never synthesize zero biomass/resource or lineage fractions from it.
- Keep selected grid coverage, biomass/resource totals, simulation time/tick, and lineage composition in the primary inspector view. Put selection IDs, accepted command position, run/scenario/parameter identity, schema version, engine/protocol, and configuration fingerprint in a collapsed native `Run provenance` disclosure; all supplied values remain exact and available when expanded.
- `RegionInspectorPanel` may accept an `emptyStateAdornment` from its shell for decorative no-selection artwork. Render it as `aria-hidden` presentation only; it must not imply an active selection, organism, or scientific measurement.
- The exact measurement/unit/time provenance note may be visually collapsed in a native disclosure to keep the inspector readable, but its text must remain available and must not be replaced by renderer-derived claims.
- Selected-cell count means authoritative simulation grid cells, not bacterial cell count. Configuration fingerprint and composed state version are identity/schema metadata; the explicit checkpoint `simulationTimeHours` field is the biological timestamp.
...
- Protocol-v9 generic lifecycle events still do not prove resistant-fraction crossing, collapse, depletion, lineage birth/extinction, or similar scientific threshold semantics. `ciprofloxacin-applied` and `model-resource-applied` prove exact typed command acceptance at their recorded biological time, not a downstream response threshold. Named scientific targets must remain visibly unsupported until runtime authority emits an explicit event vocabulary/definition that proves them.
