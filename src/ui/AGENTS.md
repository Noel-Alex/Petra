...
- `no-grid-coverage` is a resolved authoritative query outcome, not a zero-valued scientific readout or generic error. Adapters must withhold biomass/resource metrics, preserve the selection identity, and must not silently snap/enlarge the region to manufacture coverage; covered regions whose authoritative values are genuinely zero remain ordinary `ready` readouts.
