...
- Keyboard press feedback is a separate transient channel from pointer state. Native Space/Enter activation may project the same shared `press` emphasis, but adapters must never synthesize clicks or prevent default merely for motion; caller key handlers remain authoritative, matching keyup/blur/disable clears keyboard compression, and pointer cancellation must not erase an independent held keyboard press.
