...
- If control planning receives an explicit composed simulation config, every reset/reseed/replay initialize request preserves a deep copy of that config; control lifecycle must not downgrade an authoritative composed run to the synthetic fixture path.
