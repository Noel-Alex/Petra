...
- A valid preview yields an `apply-intervention` **UI intent**, not automatically a worker command. Protocol v6 exposes `apply-ciprofloxacin` for the supported antibiotic global/radial/stripe/paint vocabulary, but a UI adapter may convert a preview only when authoritative scenario/runtime metadata supplies the exact `mg/L` parameter semantics and the geometry maps losslessly to that command. Unsupported tools/geometries remain uncommittable; adapters must never substitute `synthetic-pulse` or another fixture command.
...
- Protocol-v6 generic lifecycle events still do not prove resistant-fraction crossing, collapse, depletion, lineage birth/extinction, or similar scientific threshold semantics. `ciprofloxacin-applied` proves exact command acceptance at its recorded biological time, not a downstream response threshold. Named scientific targets must remain visibly unsupported until runtime authority emits an explicit event vocabulary/definition that proves them.
