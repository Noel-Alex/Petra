...
- Large-tree SVG LOD is presentation-only and deterministic: it may hide geometry only under an explicit node budget, must render only supplied authoritative lineage nodes/edges, preserve root + selected-lineage ancestry, disclose exact hidden-descendant counts at visible frontiers, and keep the complete semantic ancestry table non-lossy. LOD must never manufacture aggregate lineages or change selection/scientific authority.
- Lineage selection is controlled presentation identity over an already-supplied lineage ID. If a requested selected ID is absent from the authoritative tree, analysis must fail closed to no selection. SVG nodes and the complete semantic ancestry table expose the same optional selection action; selection must never mutate ancestry or scientific values.
...
- Explicit ciprofloxacin phenotype detail may additionally carry MIC in `mg/L` and a supplied MIC-ratio response shift relative to a named reference genotype. Preserve those values/units verbatim; absent phenotype evidence renders `Not supplied`, and presentation must never calculate resistance or response shift from genotype labels, colors, mutation class, or relative fitness.
