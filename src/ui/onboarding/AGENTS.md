...
- App/runtime integration may provide an `AuthoritativeOnboardingGateStream` only for one exact run identity + explicit run/branch generation. Generic `advanced` events, animation callbacks, elapsed wall time, or renderer observations never satisfy a gate. A mismatched run stream is ignored.
- Replaying the guide is presentation reset only: it returns to the ecosystem stage without resetting, rewinding, reseeding, or mutating the simulation. Already-established authoritative gate facts may immediately remain satisfied on replay.
