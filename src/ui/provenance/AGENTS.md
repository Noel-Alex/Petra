...
- `localExperimentValidation.ts` is the strict bridge from one Git-tracked `experiments/results/<run-id>/<experiment-id>.json` record into one caller-selected validation lane. It validates runner/result identity and outcome consistency but never infers the lane, evidence kind, or scientific claim from experiment ids, issue labels, logs, or compact payload fields.
- The canonical `experiments/blocked.py` exit-2 compact payload projects as `blocked`, not failed scientific evidence. Other runner failures/errors/timeouts remain failed evidence for only the explicitly selected lane; a local result never creates a whole-system verdict.
