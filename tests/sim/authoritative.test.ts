...
describe('authoritative composed state', () => {
  ...
  it('rejects materially over-capacity initial composed state', () => {
    ...
    ).toThrow(/initial composed state biomass exceeds localCapacity/)
  })

  it('rejects materially over-capacity restored state atomically', () => {
    const state = createComposedState(config)
    ...
    expect(() => stepComposedState(state, config)).toThrow(
      /composed state biomass exceeds localCapacity/,
    )
    expect(state).toEqual(before)
  })

  it('accepts Float32-representable capacity boundary state across create and continue', () => {
    ...
    expect(representedTotal).toBeGreaterThan(roundedConfig.growth.localCapacity)
