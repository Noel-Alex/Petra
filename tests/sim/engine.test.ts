...
describe('SimulationEngine replay substrate', () => {
  ...
  it('rejects counted-command overflow atomically for advance and pulse', () => {
    ...
    engine.execute({ id: 'restore-max-command-count', type: 'restore', checkpoint })
    const beforeAdvance = engine.snapshot()

    expect(() =>
      engine.execute({ id: 'overflow-count-advance', type: 'advance', ticks: 1 }),
    ).toThrow(/command count.*safe integer range/)
    expect(engine.snapshot()).toEqual(beforeAdvance)

    expect(() =>
      engine.execute({ id: 'overflow-count-pulse', type: 'synthetic-pulse', magnitude: 1 }),
    ).toThrow(/command count.*safe integer range/)
    expect(engine.snapshot()).toEqual(beforeAdvance)
  })

  it('allows the last safe command count and keeps the resulting checkpoint restorable', () => {
    const engine = new SimulationEngine(identity)
    const checkpoint = structuredClone(engine.snapshot().checkpoint)
    checkpoint.commandCount = Number.MAX_SAFE_INTEGER - 1

    engine.execute({ id: 'restore-near-max-command-count', type: 'restore', checkpoint })
  ...
  it('rejects finite pulse inputs whose combined population would become non-finite', () => {
    ...
    expect(Number.isFinite(beforeOverflow.checkpoint.syntheticPopulation)).toBe(true)
