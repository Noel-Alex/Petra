...
describe('flagship release scientific identity', () => {
  ...
  it('canonicalizes raw aliases to the accepted Float32 initial state', () => {
    const first = buildFlagshipReleaseScientificIdentity({
      sourceCommitSha: SOURCE_COMMIT_SHA,
      initialization: {
        seed: baseline.seed,
        initialResourceLevel: 0.1,
        inocula: [
          {
            lineageId: 'founder-wt',
            x: 80,
    ...
    const acceptedBiomass = Math.fround(
    ...
    const second = buildFlagshipReleaseScientificIdentity({
      ...
      initialization: {
        seed: baseline.seed,
        initialResourceLevel: Math.fround(0.1),
        inocula: [
          {
            lineageId: 'founder-wt',
            x: 80,
            y: 80,
            biomass: acceptedBiomass,
          },
        ],
      },
    })

    expect(first.runInitialization).toEqual(second.runInitialization)
    expect(first.runInitialization).toEqual({
      seed: baseline.seed,
      initialResourceLevel: Math.fround(0.1),
      inocula: [
        {
    ...
    expect(first.runInitializationIdentity).toBe(
      second.runInitializationIdentity,
    )
  })

  it('keeps exact accepted run-state initialization distinct from mechanism identity', () => {
    const first = buildFlagshipReleaseScientificIdentity({
      sourceCommitSha: SOURCE_COMMIT_SHA,
      initialization: baseline,
    })
