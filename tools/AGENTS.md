...
- The same registered `expo-browser-acceptance` path owns render-publication evidence for #876. Its 1×/4×/16× runs reset to the same seed/genesis when possible and correlate accepted `ExperimentRuntime` snapshot publication → product dish projection → React dish commit. Report transaction/commit rates and latency distributions without inventing a pass threshold. Projection timing excludes the payload-estimator work, which is reported separately. Renderer-facing payload bytes remain application-data sizing estimates; do not infer structured-clone framing, Worker transport cost, bandwidth, heap/GPU memory, or justify cadence coalescing/downsampling from them alone.
