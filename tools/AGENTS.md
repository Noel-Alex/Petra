...
- Render-publication evidence in `expo-browser-acceptance` uses a separate bounded **advance-only** workload after the established renderer baseline. Group samples only by the profiler's branch/trace/tick/command-count/time correlation tuple within that workload; do not treat the tuple as a universal response id. Report runtime-snapshot→projection→React-commit timing and renderer-facing payload estimates as observational evidence only. The payload estimate is not Worker framing, bandwidth, heap, GPU memory, or a reason by itself to throttle scientific publication, lower the grid, move projection into the Worker, adopt transferables, OffscreenCanvas, WASM, WebGPU, or a backend.
