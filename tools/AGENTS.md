...
- The post-growth camera stress profile in `expo-browser-acceptance` must reset the real authoritative runtime, advance through real product controls to a fixed accepted-command frontier, pause, and then reuse the same renderer-owned camera redraw workload as the initial-state baseline. Treat accepted-command + biological-time advance as proof only of a **later-run/post-growth state**; do not label it dense/high-biomass unless a source-owned occupancy measurement is explicitly exposed. Report initial-vs-later redraw timing without inventing a pass threshold, and restore the reset scientific state before unrelated profiled workloads.
