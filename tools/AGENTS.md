...
- Render-publication evidence in `expo-browser-acceptance` uses separate bounded instrumented workloads after the established renderer baseline: an **advance-only** transaction-integrity pass plus a continuous 1×/4×/16× publication pass. The continuous publication observer is deliberately separate from playback frame/jank timing so its bookkeeping is never presented as uninstrumented renderer performance. Group samples only by the profiler's branch/trace/tick/command-count/time correlation tuple within each reset workload; do not treat the tuple as a universal response id. Report runtime-snapshot→projection→React-commit timing, phase ratios, accepted-command-normalized publication counts, and renderer-facing payload estimates as observational evidence only. Publication summaries must keep typed-array channel bytes distinct from JSON metadata bytes and surface render-event / accepted-intervention-footprint counts so mutation-heavy or intervention-heavy runs are not mischaracterized as arrays-only payloads. The payload estimate is not Worker framing, bandwidth, heap, GPU memory, or a reason by itself to throttle scientific publication, lower the grid, move projection into the Worker, adopt transferables, OffscreenCanvas, WASM, WebGPU, or a backend.
- Authoritative-playback jank profiling must drive the real product controls and same provenance-bound runtime at 1×/4×/16× from a reset initial state. A diagnostic frame distribution is publishable only when the same window both advances visible authoritative biological time and observes actual renderer WebGL draw activity; zero renderer-active frames is `blocked`/unavailable evidence, never a fast-frame pass. UI `Simulation time … h` text is rounded presentation and may prove only that the run advanced, not simulator throughput. Record main-thread task/script/layout/style/long-task symptoms and shell-geometry drift separately; Worker execution/transport timing stays in the dedicated real-Worker transport profile rather than being inferred from browser wall time. Do not invent a playback FPS/jank threshold before same-device baseline evidence exists.
- The post-growth camera stress profile in `expo-browser-acceptance` must reset the real authoritative runtime, advance through real product controls to a fixed accepted-command frontier, pause, and then reuse the same renderer-owned camera redraw workload as the initial-state baseline. Correlate the reset/later frontiers to exact composed-checkpoint `totalBiomass` and `occupiedCells` through the opt-in publication probe; missing or ambiguous frontier correlation is unavailable/blocked load evidence. Those metrics characterize authoritative model state only—they are not physical cell count/CFU, visible-pixel density, or causal proof that load changed renderer cost. Report initial-vs-later redraw timing without inventing a pass threshold, and restore the reset scientific state before unrelated profiled workloads.
- Worker transport profiling must use the real browser `WorkerSession` + composed Worker when judging #630 architecture. Keep its page isolated from product animation/render load, preserve profiling as observational only, and distinguish application-payload byte estimates, synchronous sender-side `postMessage()` handoff wall time, local accepted-snapshot clone wall time, worker execution time, and the broader non-worker request-window remainder. None of those observations is exact browser framing, receiver deserialization cost, link bandwidth, or proof that transferable buffers/downsampling are beneficial. Optimization requires before/after local evidence with deterministic authority preserved. Candidate renderer-channel estimates may additionally project the current authoritative mask/biomass/resource/drug/lineage arrays into their intended Uint8/Float32 presentation storage to quantify a lower-bound payload opportunity, but must be labelled hypothetical, arrays-only, and non-transferred until a real runtime wire contract exists.
...
- Local subprocess launchers must preserve argv arrays with `shell=False` and use `tools/local_command.py` to resolve Windows launch targets. The canonical `python` child token is pinned to the already-running interpreter before PATH lookup so Microsoft Store App Execution Aliases cannot shadow Petra's active Python; other commands resolve Windows PATH/PATHEXT shims such as `npm.cmd`. The declared command remains evidence, and a genuinely missing unrelated executable must still fail visibly.
...
- The content-support contract check validates `data/content_support/v1.json` against exact repository scenario/resource/intervention/presentation identities. It must keep research availability distinct from grounded Science-Mode admission and keep blocked expansion entries data-free except for coordination issue dependencies/reasons.
