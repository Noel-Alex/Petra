...
- Offline release preflight permits loopback serving of the built static bundle while external hostname resolution is blocked in a fresh browser profile. Required external HTML/CSS assets or observed external runtime resource loads are failures. User-triggered citation links are not startup dependencies, and an unfinished authoritative demo flow must remain `blocked` rather than being replaced with cached/demo scientific state.
