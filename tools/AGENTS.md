...
- Render-publication profiling during that same authoritative playback window may opt into the versioned app-side `__petraRenderPublicationPerformanceProbe` only from the local harness. Aggregate runtime-snapshot publication, composed→dish projection duration, renderer-facing application-payload estimates, and React dish-commit counts by the exact reset-window correlation tuple; do not persist raw frame/keyframe samples in compact evidence. Treat runtime→projection→commit ratios and correlation coverage as diagnostics for deciding whether publication coalescing is worth testing, not as an FPS target or permission to drop accepted scientific state. Renderer payload bytes remain application-payload estimates, not structured-clone framing, bandwidth, receiver deserialization, heap/GPU usage, or transfer cost. If the hooks are absent, samples are dropped, or no identities correlate, mark the evidence blocked/unavailable rather than inferring a fast path.
