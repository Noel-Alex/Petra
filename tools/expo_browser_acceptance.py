...
def summarize_render_publication_samples(
    ...
    accepted_footprint_counts: list[int] = []
    net_growth_count = 0
    ...
    for _, runtime, projection, react in complete:
        ...
        if isinstance(payload, dict):
            ...
            accepted_footprint_count = payload.get(
            ...
            if isinstance(accepted_footprint_count, int):
                accepted_footprint_counts.append(accepted_footprint_count)
    ...
    return {
        ...
        "authoritativeRenderEventCount": numeric_summary(
            [float(value) for value in event_counts]
        ),
        "acceptedInterventionFootprintCount": numeric_summary(
            [float(value) for value in accepted_footprint_counts]
        ),
        "limitation": (
            "Reset-bounded browser publication observations. Projection timing excludes the "
            "payload-estimator duration; payload bytes, metadata JSON bytes, event counts, and "
            "accepted intervention footprint counts are renderer-facing application estimates, "
            "not Worker framing, bandwidth, heap, GPU memory, event-render cost, or proof that "
    ...
    workload = cdp.eval(
        """new Promise(resolve => {
          ...
          function step(now) {
            ...
            previousOwned = canvas.dispatchEvent(event) === false;
